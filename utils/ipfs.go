...
func (u *User) UploadUserToIPFS(blockchainAddress string) string {
	if u == nil {
		return "THE BLOCKCHAIN"
	}
	file, _ := json.MarshalIndent(u, "", " ")
	_ = ioutil.WriteFile(blockchainAddress+".json", file, 0644)
	...
	jsonFile, err := os.Open(blockchainAddress + ".json")
	jsonFileCid := putFile(c, jsonFile)
	os.Remove(blockchainAddress + ".json")
	return jsonFileCid.String()
}
...
func main() {
	c, err := w3s.NewClient(
		...
		w3s.WithToken(os.Getenv("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDVDNEU1RjEyMDc5MTBBZjk1YThkY0E1MTgxMjBmQ2MzQWJEM0FhMGQiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2NTQxMTEzOTQ0NTAsIm5hbWUiOiJxb2x0eXFzaGFzaCJ9.btlU_FnsRRShI2eI4WvmHKQ75JiMPxRwUp_iZdLjYVo")),
	)
	if err != nil {
		panic(err)
	}
